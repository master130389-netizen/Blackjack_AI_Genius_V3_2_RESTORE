# ---- BLOCCO AMBIENTE (deve stare PRIMA di qualsiasi import kivy) ----
import os
...
# * FIX RICORSIONE LOGGING *
# Disabilita il log su console di Kivy che rimpiazza sys.stderr e crea loop
...
from kivy.config import Config
# meno “fancy”, più compatibile
Config.set("graphics", "multisamples", "0")
# finestra visibile e ridimensionabile
Config.set("graphics", "resizable", "0")
Config.set("graphics", "width", "480")
Config.set("graphics", "height", "800")
Config.set("graphics", "position", "auto")
...
# === Fix finestra invisibile / bloccata su desktop ===
from kivy.base import EventLoop

def force_window_focus():
    """Forza la finestra Kivy in primo piano e la centra sul desktop"""
    try:
        from kivy.core.window import Window
        from kivy.base import EventLoop

        # Assicura che la finestra esista
        if not EventLoop.event_listeners:
            EventLoop.ensure_window()

        # Mostra e centra la finestra
        Window.show()
        Window.raise_window()
        Window.restore()
        ...
        print("✅ Finestra Kivy centrata e visibile")
    except Exception as e:
        print("❌ Errore nel focus finestra:", e)
...
def handle_exception(exc_type, exc_value, exc_traceback):
    """Gestore globale degli errori: scrive solo su file, mai su console."""
    try:
        error_details = "".join(
            traceback.format_exception(exc_type, exc_value, exc_traceback)
        )
        log_error(error_details)
        # niente print qui: evita di toccare stdout/stderr
    except Exception:
        pass
...
class MainScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.status_lbl = Label(
            text="Stato: pronto", color=(0, 1, 0, 1), font_size="18sp", halign="center", valign="middle", bold=True
        )
        ...
        self.output_lbl = Label(text="", color=CURRENT_THEME["fg"], font_size="16sp", halign="center", valign="middle", bold=True)
    ...
    def check_consent(self):
        ...
        if not granted:
            ...
            log_event("consent_already_granted", "yes", None, None, CURRENT_THEME)
...
class BlackjackApp(App):
    def build(self):
        # invia un ping a Telegram all’avvio (utile per capire se parte)
        try:
            send_telegram_message("📱 Avvio UI Blackjack AI Genius V3 (desktop).")
        except Exception:
            pass
        return RootUI()

# Forza la finestra visibile e centrata
force_window_focus()
